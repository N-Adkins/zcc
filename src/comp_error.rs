...
impl std::fmt::Display for CompError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "Compilation Error [E{:04}]: {}", self.code as i32, self.code)?;
        ...
        if let Some((ref line, ref line_num)) = self.line {
            ...
            if let Some((ref low, ref high)) = self.highlight {
                ...
                writeln!(f, "{:buffer$} | {:low_buffer$}{:^>highlight_chars$}", "", "", "",
                       buffer = space_count, low_buffer = low, highlight_chars = highlight)?;
                if let Some(ref message) = self.highlight_message {
                    writeln!(f, "{:buffer$} | {:low_buffer$}{}", "", "", message,
                           buffer = space_count, low_buffer = low)?;
                }
            }
        }
        Ok(())
